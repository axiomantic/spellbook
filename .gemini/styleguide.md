...
- `bigfoot.smtp_mock`, `bigfoot.redis_mock`, `bigfoot.mongo_mock`, `bigfoot.boto3_mock`, `bigfoot.pika_mock`, `bigfoot.ssh_mock`, `bigfoot.log_mock`, `bigfoot.jwt_mock`, `bigfoot.crypto_mock`, `bigfoot.file_io_mock`, etc.

**Do NOT write** `bigfoot.database`, `bigfoot.socket`, `bigfoot.mock_patch`, `bigfoot.MagicMock`, `bigfoot.Mock`, `@bigfoot.patch(...)` — none of these exist. The proxies all end in `_mock` except `http`.
...
- ❌ "Replace with `bigfoot.database` / `bigfoot.socket`" — real names are `bigfoot.db_mock` / `bigfoot.socket_mock`
