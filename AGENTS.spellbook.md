---
name: spellbook
description: LOAD AT SESSION START. Global spellbook behavioral rules, platform detection,
  session protocol, code quality standards, subagent dispatch policies, and communication
  guidelines. Load this skill before responding to the user.
---
...
1. **On MCP-capable platforms (Claude Code, Antigravity, Codex, OpenCode, Gemini CLI):** Call `spellbook_session_init` MCP tool with `platform` = your platform value from Step 0, and `continuation_message` = user's first message (if available).
   **On Prime Agent:** Check the continual harness (`rlm.harness.overview()`) for existing session state and memories.
...
usage fast. The default roster below reflects operator-confirmed preferences (2026-08-07)
after a weekly OpenRouter budget was burned by a single `sonnet` dispatch. If a task genuinely
seems to warrant `fable`, do not silently upgrade — say so and let the operator decide. "This
is hard" is not authorization; only the operator naming `fable` is.

**EXCLUDED providers** (operator policy 2026-08-07, persisted as `prime-agent-model-preferences`
in the Prime Agent continual harness):

- `anthropic/*` — any Claude, any version. Budget-burned in a single dispatch. **Never use**
  unless operator names `opus`/`sonnet`/`haiku`/`fable` explicitly in a request.
- `google/*` — proprietary Gemini. Not in operator's "open weights" preference.
- `fable` — see the rule above.

| Kind | What it is | `model` | `effort` |
|------|-----------|---------|----------|
| **Thinking** | Planning, design, architecture, code/design review, fact-checking, adversarial review, open-ended debugging where the cause is unknown, research, synthesis, arbitration — anything requiring judgment or trade-off analysis | `openrouter/minimax/minimax-m3` ($0.30/$1.20 per M; `:batch` at half) — fallback `openrouter/deepseek/deepseek-v4-pro` | inherit session effort (omit the override) |
| **Mechanical** | Carrying out an already-approved plan or spec: TDD implementation against a written spec, completion/artifact verification against a checklist, precisely-specified amends, rote edits, running tests, git/PR/Jira mechanics, applying a described change | `openrouter/deepseek/deepseek-v4-flash-0731` ($0.09/$0.18 per M) — pinned dated snapshot, no behavior drift | `low` |
| **Code-specialist** (escalation only) | Mechanical task where `deepseek-v4-flash-0731` output fails per-file quality gate after retry | `openrouter/moonshotai/kimi-k2.7-code` ($0.70/$3.50 per M) | `low` |
...
An agent type whose frontmatter still specifies `fable` must be overridden to `minimax/minimax-m3`
at the call site until its frontmatter is updated. An agent type whose frontmatter still specifies
`opus` or `sonnet` MUST be overridden to the appropriate tier per the table above unless the
operator named `opus`/`sonnet` explicitly in the current request.
...
### Session Model Confirmation (Prime Agent / non-resumed sessions)

At session start, BEFORE any subagent dispatch, the agent MUST confirm the model
preferences with the operator. This applies to:

- Prime Agent sessions that are not handoff-resumes (the operator's first message is
  not a `boot_prompt` from a prior session).
- Any session where the operator did not name a model explicitly in their first message.

Procedure:

1. Surface the current tier-to-model mapping (the table above) in a compact summary.
2. AskUserQuestion with options: "Use defaults" / "Override one or more tiers" / "Use
   a single model for everything (specify)". For Prime Agent, give a 30s soft timeout:
   no response → apply defaults.
3. If the operator confirms or doesn't respond, persist the defaults to the session
   context (NOT to global memory — model preferences are session-scoped unless
   the operator explicitly says otherwise).
4. If they override, apply the override for this session only.

Skip this step if the operator named a specific model in their first message, or if
the session is a handoff resume where model preferences are already in the boot prompt.

This is the Prime Agent equivalent of letting the operator pick their tools. The
budget-burn incident of 2026-08-07 (a single `sonnet` dispatch exhausted the
operator's weekly OpenRouter budget) is the motivating event.

### Auto-Escalation Between Tiers

When a subagent dispatch fails quality gates (e.g. tripwire per-file gate reverts
a converted file, code review surfaces a defect the dispatched model couldn't catch),
the agent auto-escalates UP through the tier chain:

1. `deepseek-v4-flash-0731` (mechanical) → `kimi-k2.7-code` (code-specialist)
2. `kimi-k2.7-code` → `minimax-m3` (thinking)
3. `minimax-m3` → STOP, surface to operator

Auto-escalate only. NEVER auto-de-escalate DOWN. Operator may explicitly request a
cheaper tier; the agent honors that. The escalation log (model used, gate result,
reason for escalation) is included in any checkpoint or final report.
