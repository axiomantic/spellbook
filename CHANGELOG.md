...
- Philosophy: a perturbing probe proves a mechanism CAN fire; a non-perturbing
  one proves whether it DOES. Writing a sentinel or forcing a value supplies the
  conditions the mechanism needs, so a positive says the path works and nothing
  about whether the system exercises it. Both are necessary; neither
  substitutes. Recorded after a probe rewrote a buffer every cycle, recorded
  thousands of writes, and that was reported as the mechanism running -- a
  read-only pass over the same buffer found nothing written at all.
