...
description: |
  Operator-stated model preferences for subagent dispatch across all spellbook
  platforms (Claude Code, Prime Agent, OpenCode, Codex, Gemini, ForgeCode).

  This file is the SINGLE SOURCE OF TRUTH for model selection. Per-agent
  model: fields in agent frontmatter should either reference this routing or
  match it. If they conflict, this file wins; update the agent files.

  Last updated: 2026-08-08 after operator review of v5 model preferences.
  Operator: ollama local inference is slow as a CLASS, not just nemotron.
  Verifying tier: Prime Agent memory `prime_agent_model_preferences` v5.
---
