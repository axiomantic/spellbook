...
!!! note "Claude Code Data Format"
    Session data is parsed from Claude Code JSONL session files stored under `~/.claude/projects/`. Sessions from other coding assistants (OpenCode, Codex, Gemini CLI) are not currently displayed. Contributions to add session file parsing for additional platforms are welcome.
