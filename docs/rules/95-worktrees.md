...
!!! info "Optional module"
    The installer offers this module pre-checked. Config key: `rules.module.worktrees`.

Isolation between a worktree and its main checkout, the worktree location convention, and the requirement that git commands run from the worktree path.

**Why keep it:** Stops git commands from running against the wrong checkout and returning silently wrong results.

**If you decline:** The agent may run git commands from the main checkout while working in a worktree, and follows no standing worktree location convention.
