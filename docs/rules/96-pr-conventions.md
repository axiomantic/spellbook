...
!!! info "Optional module"
    The installer offers this module pre-checked. Config key: `rules.module.pr-conventions`.

How a repository's pull request template is discovered and applied, and which sections never appear in a pull request body.

**Why keep it:** Uses the repository's own PR template instead of the harness's default Test-plan shape.

**If you decline:** Pull request bodies follow the harness default, which typically imposes a Summary and Test plan structure the repository did not ask for.
