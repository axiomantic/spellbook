...
> Use when consolidating duplicated instructions across a named set of skills/commands, or detecting drifted copies of the same rule. Triggers: 'dedupe these skills', 'find duplicate instructions', 'consolidate repeated rules', 'are these instructions drifted', 'deduplicate skill group'. NOT for: finding unused code (use finding-dead-code) or general prompt review (use sharpening-prompts).
## Skill Content
