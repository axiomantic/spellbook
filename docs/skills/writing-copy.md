...
> Use when writing or de-slopping outward-facing prose: newsletters, blog posts, announcements, marketing copy, emails, release notes, doc intros, cover letters, job-application answers, resume bullets, bios. Fire this whenever you draft outward-facing prose on the author's behalf, even without a trigger phrase. Triggers: 'make this sound human', 'this reads like AI', 'de-slop this', 'remove the AI tells', 'write a newsletter', 'write a blog post', 'polish this copy'. Voice-neutral: composes with a personal voice skill that supplies identity and register. NOT for: code review (use code-review), project doc structure (use documenting-projects), READMEs (use write-readme / polish-repo), or a specific person's voice (use their voice skill).
## Skill Content
