#!/usr/bin/env bash
# bash-gate.sh - PreToolUse hook for the Bash tool
#
...
# inspects bash commands for dangerous patterns and exfiltration attempts.
# Error messages never include blocked content to prevent reflection attacks.
...
set +e
CHECK_OUTPUT="$(echo "${INPUT}" | PYTHONPATH="${PROJECT_ROOT}" python3 -m spellbook_mcp.security.check 2>>"${SPELLBOOK_SECURITY_LOG:-/dev/null}")"
CHECK_EXIT=$?
set -e
...
case ${CHECK_EXIT} in
    ...
    2)
        ...
        if [[ -n "${CHECK_OUTPUT}" ]]; then
            ...
            echo '{"error": "Security check failed: dangerous pattern detected"}'
        fi
        exit 2
        ;;
    *)
