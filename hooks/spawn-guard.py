#!/usr/bin/env python3
"""Cross-platform hook wrapper: spawn-guard.

On Unix: delegates to spawn-guard.sh.
...
def main() -> None:
    if sys.platform != "win32":
        script = HOOK_DIR / "spawn-guard.sh"
    ...
    if result.returncode == 0:
        ...
        if result.stdout.strip():
            ...
            print(json.dumps({"error": "Security check failed: blocked by policy"}))
        sys.exit(2)
    else:
        block("Security check failed: internal error")
        return
