#!/usr/bin/env bash
# spawn-guard.sh - PreToolUse hook for spawn_claude_session
#
# Claude Code Hook Protocol:
#   Receives JSON on stdin: {"tool_name": "...", "tool_input": {"prompt": "..."}}
...
# inspects the spawn prompt for injection and escalation patterns.
...
# ---------------------------------------------------------------------------
# Claude Code sends the full MCP tool name (e.g., "mcp__spellbook__spawn_claude_session")
# but check.py routes on bare names (e.g., "spawn_claude_session"). Since this hook
# is specifically for spawn_claude_session, normalize to ensure correct rule routing.
...
case ${CHECK_EXIT} in
    ...
    2)
        ...
        if [[ -n "${CHECK_OUTPUT}" ]]; then
            ...
            echo '{"error": "Security check failed: blocked by policy"}'
        fi
        exit 2
        ;;
    *)
