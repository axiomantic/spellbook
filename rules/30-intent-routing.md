...
**Standing exception — the Nord Modular G2 project.** In `axiomantic/nmg2-tools`, `axiomantic/nmg2-artifacts`, `axiomantic/mcf5307`, `axiomantic/mc68k`, `axiomantic/dsp56300`, `axiomantic/gearmulator` and `axiomantic/G2-Edit`, I select `skip develop entirely` at the entry gate. This is standing, for every session, and it does not need restating per session. Those repositories carry an `AGENTS.md` and a roadmap that define the process, and that process is the whole of it. Do not invoke `develop` there. Do not add a phase, a gate, an approval step or a checkpoint ladder to what those documents describe, and do not build a linter, a register or an audit script for them — that project removed seven such instruments deliberately and they are not to come back. This overrides, in those repositories only, the general allowance elsewhere in these rules for mechanizing a verification table or pinning a count that a test checks. That is the exact shape that kept regrowing there: a pinned number fails whenever the thing it measures improves, so it converts progress into a red build and someone into a maintainer of the pin. If something in those repositories looks like it needs checking, say so and let me decide. Do not write the check.
