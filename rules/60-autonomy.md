...
benefit: >
  Keeps an autonomous run from stopping early or giving up on the first failure.
declining_means: >
  The agent may end a turn at a checkpoint instead of continuing, may report an
  environment constraint after a single failed attempt, and may expand scope
  without pausing.
related:
  - skills/develop
