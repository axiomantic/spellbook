...
default: "on"
description: >
  A stated action is executed in the same turn that states it, and the tool
  call precedes the prose that reports it.
benefit: >
  Closes the gap where an announcement reads exactly like the work, so an
  unexecuted intent cannot end a turn wearing the costume of a completed one.
declining_means: >
  The agent may end a turn having announced work it did not start, and the
  user cannot tell that from a turn where the work was done.
related:
