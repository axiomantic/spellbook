"""SQLAlchemy engine and session factory for spellbook database.

Uses an async engine with NullPool (avoids stale pooled connections,
appropriate for SQLite) and a 5-second busy timeout to mitigate
"database is locked" contention under concurrent writes.
...
# Register PRAGMAs
event.listen(spellbook_engine.sync_engine, "connect", _setup_pragmas)

# Session factory (expire_on_commit=False for detached use in route handlers)
