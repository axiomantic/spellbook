...
def test_gemini_install_includes_extension_skills(tmp_path):
    """Test that install method creates extension skills."""
    # Mock gemini CLI availability
    def mock_check_cli():
        return True

    check_gemini_cli_available_mock = tripwire.mock("installer.platforms.gemini:check_gemini_cli_available")
    check_gemini_cli_available_mock.calls(mock_check_cli)
    check_gemini_cli_available_mock.calls(mock_check_cli)
    ...
    check_gemini_cli_available_mock.assert_call(args=(), kwargs={})
    check_gemini_cli_available_mock.assert_call(args=(), kwargs={})
